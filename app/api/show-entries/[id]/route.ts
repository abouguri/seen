...
/** Mirrors app/api/entries/[id]/route.ts — no tags. */
...
function mapRow(data: {
  ...
  return {
    ...
    precision: data.precision as ShowWatchEntry["precision"],
...
export async function DELETE(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { data, error } = await supabase.from("show_watch_entries").delete().eq("id", id).select("id");
